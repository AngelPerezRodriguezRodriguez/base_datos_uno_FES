...
/*
Cargamos el esquema y el estado inicial de la B.D. 'colegio2459'
...
-- No son datos de ninguna tabla de la B.D. 'colegio2459'
...
SELECT 
    nombre, ap_paterno AS paterno, ap_materno AS materno, curp
FROM
    alumnos;
...
FROM
    alumnos
WHERE
    sexo = 'f' AND ciudad = 'queretaro'
ORDER BY nombre;
-- ... que viven en 'queretaro'

SELECT 
    nombre, ap_paterno, ap_materno, sexo, ciudad
FROM
    alumnos
WHERE
    sexo = 'f' AND ciudad = 'queretaro'
...
expression #1 of SELECT list contains nonaggregated column 'colegio2459.alumnos.sexo'; 
...
WHERE
    LENGTH(curp) = 18
ORDER BY 2 , 3 , 1;
-- ... con los registros ordenados por apellido paterno, apellido materno y nombre
