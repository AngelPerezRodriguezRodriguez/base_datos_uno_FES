...
SELECT 
    COUNT(*), SUM(peso), AVG(peso), MAX(peso), MIN(peso)
FROM
    alumnos;
...
WHERE
    peso > 0
GROUP BY ciudad , sexo
HAVING prom_peso >= 40 AND prom_peso <= 50
ORDER BY prom_peso DESC;
-- ... con el promedio del peso mayor o igual a cuarenta pero menor o igual a cincuenta
...
FROM
    alumnos
WHERE
    peso IS NOT NULL AND peso > 0
        AND estatura IS NOT NULL
        AND estatura > 0
ORDER BY imc;
...
WHERE
    peso IS NOT NULL AND peso > 0
        AND estatura IS NOT NULL
        AND estatura > 0
        AND fedita >= '2017-01-01 00:00:00'
        AND fedita <= '2017-12-31 23:59:59'
ORDER BY imc;
...
WHERE
    peso IS NOT NULL AND peso > 0
        AND estatura IS NOT NULL
        AND estatura > 0
        AND NOT (fedita >= '2017-01-01 00:00:00'
        AND fedita <= '2017-12-31 23:59:59')
ORDER BY imc;
...
SELECT 
    nombre, ap_paterno, ap_materno, peso, estatura
FROM
    alumnos
WHERE
    peso >= 90 AND peso <= 100
ORDER BY peso DESC;
