...
SELECT 
    nombre, LENGTH(nombre) AS long_nombre
FROM
    alumnos
...
SELECT 
    LENGTH(nombre) AS long_nombre, COUNT(*)
FROM
    alumnos
...
SELECT 
    nombre, LOCATE('a', nombre) AS primera_a
FROM
    alumnos
ORDER BY nombre;
...
FROM
    alumnos
WHERE
    LOCATE('a', nombre, LOCATE('a', nombre) + 1) > 0;
-- ... que tengan una segunda 'a' en el nombre

SELECT 
    nombre, LOCATE('maria', nombre) AS maria
FROM
    alumnos
WHERE
    LOCATE('maria', nombre) > 0;
-- ... que tengan 'maria' en el nombre



SELECT 
    nombre, LEFT(nombre, 4), RIGHT(nombre, 4), MID(nombre, 4)
FROM
    alumnos;
/*
Obtenemos los nombres de los alumnos,
...
SELECT 
    nombre, LEFT(nombre, 4), RIGHT(nombre, 4), MID(nombre, 4, 4)
FROM
    alumnos;
...
SELECT 
    nombre, sexo, IF(sexo = 'f', 'femenino', 'no')
FROM
    alumnos;
...
FROM
    alumnos;
-- Concatenamos los campos que conforman el nombre
...
SELECT 
    CONCAT_WS(' ', nombre, ap_paterno, ap_materno) AS nombre_completo
FROM
    alumnos;
...
SELECT 
    CONCAT_WS(' ', nombre, ap_paterno, ap_materno) AS nombre_completo,
    LENGTH(CONCAT_WS(' ', nombre, ap_paterno, ap_materno)) AS long_nombre_completo
FROM
    alumnos
...
SELECT 
    nombre,
    REPLACE(REPLACE(REPLACE(REPLACE(nombre, 'A', '4'),
                'E',
                '3'),
            'I',
            '1'),
        'O',
        '0') AS vocales_numeros
FROM
    alumnos;
/*
... remplazamos 'E' por '3'
...
UPDATE alumnos 
SET 
    email = LOWER(CONCAT(LEFT(nombre, 1),
                    '_',
                    REPLACE(ap_paterno, ' ', '_'),
                    '@gmail.com'))
WHERE
...
SELECT 
    ...
    MD5(CONCAT(nombre,
                    ap_paterno,
                    ap_materno,
                    SYSDATE())) AS contrasenia
FROM
    alumnos
ORDER BY nombre , ap_paterno , ap_materno;
-- ... una contraseña con base en MD5 con los campos del nombre y la fecha actual
...
FROM
    alumnos
ORDER BY nombre , ap_paterno , ap_materno;
-- ... una contraseña con base en los últimos dieciocho caracteres de la cadena MD5