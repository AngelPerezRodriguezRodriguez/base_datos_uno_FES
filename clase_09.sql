...
no es lo mismo que
Obtener todos los salones (sentencia diferente LEFT JOIN).
*/
...
pero se puede utilizar. Sin ON O USING realizaos un producto cartesiano.
*/
...
El propio manejador infiere cuales cuál es la PK y la FK.

A diferencia de USING, sólo lo podemos utilizar una vez.
Si en el ejemplo anterior utilizamos NATURAL en vez USING, el manejador 
...
FROM
pagos p 
RIGHT JOIN alumnos a ON(p.clave_alu = a.clave_alu)
...
deben incluir el campo utilizado en clausula ON, USING o NATURAL.
*/
